...
class TPS:       
    ...
    @staticmethod
    def d(a, b):
        result = np.sqrt(np.square(a[:, None, :2] - b[None, :, :2]).sum(-1))
        # print("a", a.shape, a)
        # print("b", b.shape, b)
        # print("d(a,b)",result.shape, result)
        return result
...
def tensor2_warp_image_cv(img_tensor, c_src_tensor, c_dst_tensor, dshape=None):
    ...
    result = torch.from_numpy(result).permute(2,0,1).unsqueeze(0).to(img_tensor.device)
    return result