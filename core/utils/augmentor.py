...
class FlowAugmentor:
    ...
    def spatial_transform(self, img1, img2, flow):
        ...
        # check img1.shape[0] - self.crop_size[0] > 0
        if (img1.shape[0] - self.crop_size[0]) < 0:
            # resize imgs , flow to crop_size
            _scale = self.crop_size[0] / img1.shape[0]
            img1 = cv2.resize(img1, None, fx=_scale, fy=_scale, interpolation=cv2.INTER_LINEAR)
            img2 = cv2.resize(img2, None, fx=_scale, fy=_scale, interpolation=cv2.INTER_LINEAR)
            flow = cv2.resize(flow, None, fx=_scale, fy=_scale, interpolation=cv2.INTER_LINEAR)
            flow = flow * [_scale, _scale]
        # check img1.shape[1] - self.crop_size[1] > 0
        if (img1.shape[1] - self.crop_size[1]) < 0:
            _scale = self.crop_size[1] / img1.shape[1]
            img1 = cv2.resize(img1, None, fx=_scale, fy=_scale, interpolation=cv2.INTER_LINEAR)
            img2 = cv2.resize(img2, None, fx=_scale, fy=_scale, interpolation=cv2.INTER_LINEAR)
            flow = cv2.resize(flow, None, fx=_scale, fy=_scale, interpolation=cv2.INTER_LINEAR)
            flow = flow * [_scale, _scale]
